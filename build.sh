#!/bin/bash
# Builds YTBar.app (and optionally a .dmg with: ./build.sh --dmg).
set -euo pipefail
cd "$(dirname "$0")"

APP="build/YTBar.app"
BIN="$APP/Contents/MacOS/YTBar"
...
<plist version="1.0">
<dict>
  <key>CFBundleName</key><string>YTBar</string>
  <key>CFBundleDisplayName</key><string>YTBar</string>
  <key>CFBundleIdentifier</key><string>com.local.ytbar</string>
  <key>CFBundleExecutable</key><string>YTBar</string>
  ...
  <key>NSAppleEventsUsageDescription</key><string>YTBar reads the playback position of YouTube tabs in Google Chrome.</string>
</dict>
</plist>
PLIST

# Bundle the app icon if one is present (the app draws its own menu bar icon regardless).
mkdir -p "$APP/Contents/Resources"
[[ -f assets/AppIcon.icns ]] && cp assets/AppIcon.icns "$APP/Contents/Resources/AppIcon.icns"

# --- Signing / notarization ---
# For a clean (no Gatekeeper warning) release, set up once on this Mac and export TEAM_ID:
#   1. A "Developer ID Application" certificate in your keychain (Xcode > Settings > Accounts).
#   2. A notarytool credential profile:
#        xcrun notarytool store-credentials "ytbar" \
#          --apple-id you@example.com --team-id YOUR_TEAM_ID --password <app-specific-password>
# Then `TEAM_ID=YOUR_TEAM_ID ./build.sh --dmg` auto-signs + notarizes. Without a cert it falls
# back to an ad-hoc dev build (runnable locally; users would need right-click > Open once).
TEAM_ID="${TEAM_ID:-}"
NOTARY_PROFILE="${NOTARY_PROFILE:-ytbar}"
...
if [[ "${1:-}" == "--dmg" ]]; then
  ...
  # Eject any stale "YTBar" volumes from earlier builds first. Otherwise a name
  # collision mounts this one as "YTBar 2", the hardcoded /Volumes path below points
  # at the wrong volume (layout capture silently fails), and the stale mounts pile up in Finder.
  for d in $(hdiutil info | awk '/YTBar/ {print $1}'); do hdiutil detach "$d" >/dev/null 2>&1 || true; done

  # Lay out the window on a read-write image to capture its .DS_Store, then build the final
  # image from the folder (see below).
  hdiutil create -volname "YTBar" -srcfolder "$STAGE" -ov -format UDRW build/rw.dmg >/dev/null
...
tell application "Finder"
  tell disk "YTBar"
    ...
    set position of item "YTBar.app" of container window to {130, 150}
...
OSA
  ...
  cp "/Volumes/YTBar/.DS_Store" "$STAGE/.DS_Store" 2>/dev/null || true
  ...
  hdiutil create -volname "YTBar" -srcfolder "$STAGE" -ov -format UDZO "$DMG" >/dev/null
  rm -rf "$STAGE"
  ...
  stray="$(find "/Volumes/YTBar" -maxdepth 1 -name ".*" ! -name ".DS_Store" 2>/dev/null)"
